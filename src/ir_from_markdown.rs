...
impl DocumentIR {
    ...
    pub fn from_markdown(markdown: &str, format: DocumentFormat) -> Self {
        ...
        DocumentIR {
            metadata: Metadata { format, title: None },
            sections,
        }
    }
}
...
impl<'a> MarkdownParser<'a> {
    ...
    fn parse_sections(&mut self) -> Vec<Section> {
        ...
        let mut current = Section { title: None, elements: Vec::new() };
        ...
        while self.pos < self.lines.len() {
            ...
            if is_thematic_break(line) {
                ...
                if !current.elements.is_empty() || current.title.is_some() {
                    ...
                    current = Section { title: None, elements: Vec::new() };
                }
                continue;
            }
            ...
            if let Some((level, text)) = parse_atx_heading(line) {
                ...
                if level == 1 {
                    ...
                    current = Section { title: Some(text.clone()), elements: Vec::new() };
                } else {
                    current
                        .elements
                        .push(Element::Heading(Heading { level, content: parse_inline(&text) }));
                }
                continue;
            }
        ...
        if sections.is_empty() {
            sections.push(Section { title: None, elements: Vec::new() });
        }
    ...
    fn parse_paragraph(&mut self) -> Paragraph {
        ...
        let text = lines.join(" ");
        Paragraph { content: parse_inline(&text) }
    }
    ...
    fn parse_table(&mut self) -> Option<Table> {
        ...
        for (i, row_line) in data_rows.iter().enumerate() {
            let cells = split_pipe_row(row_line)
                ...
                .collect();
            rows.push(TableRow { cells, is_header: i == 0 });
        }
