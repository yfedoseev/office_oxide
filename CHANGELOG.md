...
Plus: 1,347 library and workspace tests green (`--all-features`), clippy `-D warnings` clean, rustdoc `-D warnings` clean, `cargo fmt --check` clean, `cargo audit` and `cargo deny` clean, C# (xUnit runner 4.0) and Node (koffi 3.3.1) suites green, and the `house-rules` gate green.
