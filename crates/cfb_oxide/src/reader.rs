...
use crate::directory::{parse_directory, DirEntry, EntryType, NO_ENTRY};
use crate::error::{CfbError, Result};
use crate::header::{CfbHeader, MAX_REG_SECT};
...
fn read_fully<R: Read>(reader: &mut R, buf: &mut [u8]) -> crate::error::Result<usize> {
    let mut total = 0;
    while total < buf.len() {
...
mod tests {
    use super::*;
    use crate::header::{END_OF_CHAIN, FAT_SECT, FREE_SECT};
    use std::io::Cursor;
