            ...
            match reader.read_resolved_event()? {
                (ref resolve, Event::Start(ref e)) => {
                    if xml::matches_ns(resolve, pml)
                        && e.local_name().as_ref() == b"sldIdLst"
                    {
                        slides = parse_slide_id_list(&mut reader)?;
                    }
                }
                (ref resolve, Event::Empty(ref e)) => {
                    if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"sldSz" {
...
fn parse_slide_id_list(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Vec<SlideId>> {
    let pml = xml::ns::PML;
    let mut slides = Vec::new();
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) | (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"sldId" {
                    let id: u32 = xml::optional_attr_str(e, b"id")?
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(0);
                    let rel_id = xml::required_attr_str(e, b"r:id")?.into_owned();
                    slides.push(SlideId { id, rel_id });
                }
            }
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"sldIdLst" {
                    break;
                }
            }
            (_, Event::Eof) => break,
