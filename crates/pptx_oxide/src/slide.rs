...
use office_core::relationships::{Relationships, TargetMode};
use office_core::xml;
...
type CoreResult<T> = office_core::Result<T>;
...
/// Create a namespace-aware reader that does NOT trim text content.
fn make_content_reader(xml_data: &[u8]) -> quick_xml::NsReader<&[u8]> {
    quick_xml::NsReader::from_reader(xml_data)
}
        ...
        loop {
            match reader.read_resolved_event()? {
                (ref resolve, Event::Start(ref e)) => {
                    if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"spTree" {
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, pml) {
                    match e.local_name().as_ref() {
                        b"nvSpPr" => {
                            let props = parse_nv_common_props(reader, pml)?;
                            id = props.0;
                            name = props.1;
                            alt_text = props.2;
                            placeholder = props.3;
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"sp" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_picture(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Shape> {
    let pml = xml::ns::PML;
    let mut id = 0u32;
    let mut name = String::new();
    let mut alt_text = None;
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, pml) {
                    match e.local_name().as_ref() {
                        b"nvPicPr" => {
                            let props = parse_nv_pic_props(reader, pml)?;
                            id = props.0;
                            name = props.1;
                            alt_text = props.2;
                        }
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"pic" {
                    break;
                }
            }
            (_, Event::Eof) => break,
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, pml) {
                    match e.local_name().as_ref() {
                        b"nvGrpSpPr" => {
                            let props = parse_nv_grp_props(reader, pml)?;
                            id = props.0;
                            name = props.1;
                        }
                        b"grpSpPr" => {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"grpSp" {
                    break;
                }
            }
            (_, Event::Eof) => break,
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, pml) {
                    match e.local_name().as_ref() {
                        b"nvGraphicFramePr" => {
                            let props = parse_nv_graphic_frame_props(reader, pml)?;
                            id = props.0;
                            name = props.1;
                        }
                        b"xfrm" => {
                ...
                } else if xml::matches_ns(resolve, a) {
                    match e.local_name().as_ref() {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"graphicFrame" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_graphic_data_table(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"tbl" {
                    let table = parse_table(reader, rels)?;
                    // Skip to end of graphicData
                    skip_to_end_of(reader, a, b"graphicData")?;
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"graphicData" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_connector(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Shape> {
    let pml = xml::ns::PML;
    let mut id = 0u32;
    let mut name = String::new();
    let mut position = None;
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, pml) {
                    match e.local_name().as_ref() {
                        b"nvCxnSpPr" => {
                            let props = parse_nv_cxn_props(reader, pml)?;
                            id = props.0;
                            name = props.1;
                        }
                        b"spPr" => {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"cxnSp" {
                    break;
                }
            }
            (_, Event::Eof) => break,
    ...
    Ok(Shape::Connector(ConnectorShape {
        id,
        name,
        position,
    }))
...
fn parse_nv_common_props(
    ...
    loop {
        match reader.read_resolved_event()? {
                    ...
                    match e.local_name().as_ref() {
                        b"cNvPr" => {
                            ...
                            name = xml::optional_attr_str(e, b"name")?
                                .map(|v| v.into_owned())
                                .unwrap_or_default();
                            alt_text = xml::optional_attr_str(e, b"descr")?
                                .map(|v| v.into_owned());
                            xml::skip_element(reader)?;
                        }
                    ...
                    match e.local_name().as_ref() {
                        ...
                        b"ph" => {
                            placeholder = Some(PlaceholderInfo {
                                ph_type: xml::optional_attr_str(e, b"type")?
                                    .map(|v| v.into_owned()),
                                idx: xml::optional_attr_str(e, b"idx")?
                                    .and_then(|v| v.parse().ok()),
                            });
                        }
                        _ => {}
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"nvSpPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) | (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"cNvPr" {
                    id = xml::optional_attr_str(e, b"id")?
                        .and_then(|v| v.parse().ok())
                        .unwrap_or(0);
                    name = xml::optional_attr_str(e, b"name")?
                        ...
                        .unwrap_or_default();
                    alt_text =
                        xml::optional_attr_str(e, b"descr")?.map(|v| v.into_owned());
                }
            }
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"nvPicPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_nv_grp_props(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"nvGrpSpPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_nv_graphic_frame_props(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml)
                    && e.local_name().as_ref() == b"nvGraphicFramePr"
                {
                    break;
                }
            }
...
fn parse_nv_cxn_props(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"nvCxnSpPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_shape_properties(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"xfrm" {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"spPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_grp_shape_properties(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"grpSpPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_xfrm(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Option<ShapePosition>> {
    Ok(Some(parse_xfrm_contents(reader)?))
}

/// Parse the contents of an `a:xfrm` or `p:xfrm` element: `<a:off x y/>`, `<a:ext cx cy/>`.
fn parse_xfrm_contents(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<ShapePosition> {
    let a = xml::ns::DRAWING_ML;
    let mut x = 0i64;
    let mut y = 0i64;
    let mut cx = 0i64;
...
fn parse_text_paragraph(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"p" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_text_run(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"rPr" {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"r" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_run_properties(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) | (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"hlinkClick" {
...
fn parse_text_field(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"t" {
                    text = xml::read_text_content(reader)?;
                } else {
                    xml::skip_element(reader)?;
                }
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"fld" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_row(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"tc" {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"tr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_cell(
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, a) && e.local_name().as_ref() == b"txBody" {
                    text_body = Some(parse_text_body(reader, rels)?);
                } else {
                    xml::skip_element(reader)?;
                }
...
pub(crate) fn extract_notes_text(xml_data: &[u8]) -> Option<String> {
    ...
    loop {
        match reader.read_resolved_event() {
            Ok((ref resolve, Event::Start(ref e))) => {
                if xml::matches_ns(resolve, pml) && e.local_name().as_ref() == b"spTree" {
      ...
      <a:tbl>
        ...
        if let Shape::GraphicFrame(ref gf) = slide.shapes[0] {
            ...
            if let GraphicContent::Table(ref tbl) = gf.content {
                ...
                let cell_text = extract_plain_text_from_body(
                    tbl.rows[0].cells[0].text_body.as_ref().unwrap(),
                );
