...
use crate::formatting::{parse_paragraph_properties, parse_run_properties};
use crate::headers::{HeaderFooterRef, PageOrientation};
use crate::table::{
    MergeType, Shading, TableCellProperties, TableRowProperties, TableWidth, TableWidthType,
};

// Use office_core::Result internally for all XML parsing (it has From<quick_xml::Error>).
// DocxError wraps office_core::Error, so conversion at the public boundary is automatic via `?`.
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"body" {
                    in_body = false;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_paragraph(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Paragraph> {
    let wml = xml::ns::WML;
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, wml) {
                    match e.local_name().as_ref() {
                        b"pPr" => {
                            paragraph.properties = Some(parse_paragraph_properties(reader)?);
                        }
                        b"r" => {
                            paragraph
                                .content
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"p" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_run(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Run> {
    let wml = xml::ns::WML;
        ...
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                if xml::matches_ns(resolve, wml) {
                    match e.local_name().as_ref() {
                        b"rPr" => {
                            run.properties = Some(parse_run_properties(reader)?);
                        }
                        b"t" => {
                            let text = xml::read_text_content(reader)?;
                            if !text.is_empty() {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"r" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_hyperlink(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"hyperlink" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_drawing(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Option<DrawingInfo>> {
    let wp_ns = b"http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing";
    let a_ns = xml::ns::DRAWING_ML;
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) => {
                ...
                if matches!(resolve, quick_xml::name::ResolveResult::Bound(ns) if ns.as_ref() == wp_ns)
                {
                    match local_bytes {
                ...
                } else if matches!(resolve, quick_xml::name::ResolveResult::Bound(ns) if ns.as_ref() == a_ns)
                    && local_bytes == b"blip"
                {
...
fn parse_table(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Table> {
    let wml = xml::ns::WML;
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tbl" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_properties(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<TableProperties> {
    let wml = xml::ns::WML;
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tblPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_grid(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<Vec<office_core::units::Twip>> {
    let wml = xml::ns::WML;
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) | (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"gridCol" {
                    if let Ok(Some(w)) = xml::optional_attr_str(e, b"w:w") {
                        let val: i32 = w.parse().unwrap_or(0);
                        cols.push(office_core::units::Twip(val));
                    }
                }
            }
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tblGrid" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_row(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<TableRow> {
    let wml = xml::ns::WML;
    let mut properties = None;
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_row_properties(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<TableRowProperties> {
    let wml = xml::ns::WML;
    ...
    loop {
        match reader.read_resolved_event()? {
            (ref resolve, Event::Start(ref e)) | (ref resolve, Event::Empty(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tblHeader" {
                    props.is_header = true;
                }
            }
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"trPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_cell(reader: &mut quick_xml::NsReader<&[u8]>) -> CoreResult<TableCell> {
    let wml = xml::ns::WML;
    let mut properties = None;
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tc" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_table_cell_properties(
    ...
    loop {
        match reader.read_resolved_event()? {
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"tcPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
...
fn parse_section_properties(
    ...
    loop {
        match reader.read_resolved_event()? {
                    ...
                    match e.local_name().as_ref() {
                        b"pgSz" => {
                            ...
                            props.page_size = Some(PageSize {
                                width: office_core::units::Twip(w),
                                height: office_core::units::Twip(h),
                                orient,
                            });
                        }
                        b"pgMar" => {
                            props.margins = Some(PageMargins {
                                top: office_core::units::Twip(
                                    xml::optional_attr_str(e, b"w:top")?
                                        .and_then(|v| v.parse().ok())
                                        .unwrap_or(1440),
                                ),
                                bottom: office_core::units::Twip(
                                    xml::optional_attr_str(e, b"w:bottom")?
                                        .and_then(|v| v.parse().ok())
                                        .unwrap_or(1440),
                                ),
                                left: office_core::units::Twip(
                                    xml::optional_attr_str(e, b"w:left")?
                                        .and_then(|v| v.parse().ok())
                                        .unwrap_or(1440),
                                ),
                                right: office_core::units::Twip(
            ...
            (ref resolve, Event::End(ref e)) => {
                if xml::matches_ns(resolve, wml) && e.local_name().as_ref() == b"sectPr" {
                    break;
                }
            }
            (_, Event::Eof) => break,
