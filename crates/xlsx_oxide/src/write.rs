...
use office_core::relationships::rel_types;
...
impl XlsxWriter {
    ...
    fn build_workbook_xml(&self, sheet_rids: &[String]) -> office_core::Result<Vec<u8>> {
    ...
    fn build_worksheet_xml(sheet: &SheetData) -> office_core::Result<Vec<u8>> {
        let mut w = Writer::new_with_indent(Vec::new(), b' ', 2);
    ...
    fn build_styles_xml() -> office_core::Result<Vec<u8>> {
        let mut w = Writer::new_with_indent(Vec::new(), b' ', 2);
