...
fn main() {
    ...
    let mut writer = XlsxWriter::new();
    {
        ...
        for (i, (name, price, qty)) in rows.iter().enumerate() {
            ...
            sheet.set_cell_styled(row, 1, CellData::Number(*price),
            ...
            sheet.set_cell_styled(row, 3, CellData::Formula(formula),
                CellStyle::new().number_format(NumberFormat::Currency));
        }

        // Totals row
        let total_row = rows.len() + 1;
        sheet.set_cell_styled(total_row, 0, CellData::String("TOTAL".into()),
            CellStyle::new().bold());
        sheet.set_cell_styled(total_row, 3,
