...
<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main">
  <w:body>
    ...
    if let Element::List(list) = &ir.sections[0].elements[0] {
        ...
        let item0_text: String = list.items[0].content.iter().filter_map(|e| {
            if let Element::Paragraph(p) = e {
                Some(p.content.iter().filter_map(|c| if let InlineContent::Text(s) = c { Some(s.text.as_str()) } else { None }).collect::<String>())
            } else { None }
        }).collect();
        assert!(item0_text.contains("First item"), "item0: {item0_text}");
        let item1_text: String = list.items[1].content.iter().filter_map(|e| {
            if let Element::Paragraph(p) = e {
                Some(p.content.iter().filter_map(|c| if let InlineContent::Text(s) = c { Some(s.text.as_str()) } else { None }).collect::<String>())
            } else { None }
        }).collect();
